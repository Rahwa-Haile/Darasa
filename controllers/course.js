...
const updateCourse = async (req, res) => {
  try {
    const courseId = req.params.id 
    const course = await Course.findOneAndUpdate({_id:courseId}, req.body, {new: true, runValidators: true})
    res.status(200).json({course})
  } catch (error) {
    console.log(error);
  }
...
const deleteCourse = async (req, res) => {
  try {
    ...
    res.status(200).json({msg: `course ${course.courseTitle} has been deleted`})
  } catch (error) {
    console.log(error);
  }
};
